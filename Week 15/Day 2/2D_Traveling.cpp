...
ll taka(ll x1, ll y1, ll x2, ll y2)
{
...
void solve()
{
    ...
    a--, b--;
    ll res = taka(arr[a].fi, arr[b].fi, arr[a].sc, arr[b].sc);
    ll x = 4e9 + 5, y = 4e9 + 5;
    f(i, 0, k)
    {
        x = min(x, taka(arr[a].fi, arr[i].fi, arr[a].sc, arr[i].sc));
        y = min(y, taka(arr[b].fi, arr[i].fi, arr[b].sc, arr[i].sc));
    }
    res = min(res, x + y);
    cout << res << ee;
}
